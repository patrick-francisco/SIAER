...
Nesta seção serão abordados os detalhes sobre a implementação do protocolo do comunicação proposto no capítulo \ref{cap:comunicacaosemfio}. O objetivo é explicar os métodos que levam o sistema ao seu funcionamento desejado. Busca-se fazer isso de forma clara e objetiva, separando o sistema em blocos.
...
Os microprocessadores da família CC430, assim como os MSP430, possuem um sistema de proteção contra travamentos. Esta proteção é conhecida como \textit{Watchdog}. Ele atua como um contador, o qual deve ser resetado durante a execução do programa. Caso o programa fique preso em algu lugar, o contador do Watchdog estoura e isso dispara a reinicialização do microcontrolador.
...
Caso ele encontre algum ônibus, há a alocação de algumas variáveis e o sistema ônibus e do guichê entram num estado estável de comunicação. Nesta etapa eles trocam mensagens a cada segundo, buscando manter a conexão e zerando suas variáveis de TIMEOUT. Ao mesmo tempo, o guichê está ativo para receber novas conexões de ônibus. Mais detalhes acerca da troca de dados sobre encomendas podem ser encontrados no ítem ~\ref{subsectionprocessamento}.
...
Informações mais detalhadas sobre a troca de mensagens via RF encontram-se no ítem 3.4.3.3.
