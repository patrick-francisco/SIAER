...
\begin{enumerate}
		...
		Ao iniciar sua viagem o aplicativo do ônibus estará automaticamente conectado ao dispositivo que viabiliza a comunicação \textit{wireless} o, CC430F6137, através de uma comu-nicação serial RS-232, detalhada na seção~\ref{subsection:InterfaceMSP}.
...
\subsection{Interface com o Microcontrolador CC430F6137}\label{subsection:InterfaceMSP}
...
O CC430F6137 é o microcontrolador, fabricado pela empresa \textit{Texas Instruments}, utilizado neste projeto que viabiliza a comunicação \textit{wireless}. O detalhamento sobre o \textit{hardware} de comunicação \sigla{RF}{\textit{Radio Frequency}} pode ser encontrado na seção ~\ref{hardware}.

O protocolo de comunicação que permite o interfaceamento entre os aplicativos de \textit{software} e o microntrolador CC430F6137 é o RS-232, o mesmo protocolo utilizado para a comunicação com o leitor óptico de código de barras BR-310 (seção~\ref{section:confcomserial}). As configurações realizadas em ambos os dispositivos são as mesmas apresentadas na tabela~\ref{tableRS232}, utilizadas para interface com o leitor. Desta maneira, tanto as aplicações desenvolvidas em linguagem C\#.NET, quanto o \textit{firmware} que roda no microcontrolador CC430F6137, trabalham utilizando os parâ- metros da tabela~\ref{tableRS232} para envio e recebimento de dados pela serial.
