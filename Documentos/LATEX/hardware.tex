...
O Sistema Integrado de Acompanhamento de Encomendas Rodoviárias utilizou, inicialmente, o kit de desenvolvimento EZ430-RF2500, da Texas Instruments. Esta decisão foi fundada na disponibilidade, baixo custo e possibilidade tanto de trabalhar com RF quanto de se comunicar via UART presentes no kit. O problema desta plataforma de desenvolvimento se dá na freqüência de operação de sua comunicação sem fio (2.4GHz) e no tamanho de sua antena. Esses fatores levam tal kit a ser um excelente escolha caso a distância em um transmissor e um receptor for pequena (até 10 metros). No caso do projeto SIAER, a distância entre guichê e ônibus é bem significativa, e um sistema de comunicação mais robusto se faz necessário.

Através de contatos com a filial da Texas Instruments em Freising, Alemanha, uma parceria foi criada. Recebeu-se gratuitamente os kits de desenvolvimento necessários e o compilador Code Composer para que se pudesse tornar viável as melhorias sugeridas no início deste relatório. Em contrapartida, deveria-se usar preferencialmente as ferramentas daquela empresa e no fim do projeto reportar os resultados, adicionando as experiências obtidas e sugerindo possíveis melhorias. O kit de desenvolvimento escolhido foi o EM430F6137RF900, no qual estão inclusos, segundo \cite{EM430F6137RF900}:
...
Para a comunicação serial utiliza-se um conversor Serial-USB TUSB3410. Este circuito proporciona a conectividade entre um dispositivo serial e uma interface USB. O diagrama a seguir representa tal dispositivo ~\ref{fig:TUSB3410}.
...
\begin{itemize}
	...
	\item Arquitetura RISC de 16 bits, memória estendida, clock de até 20MHz
...
O software utilizado para o layout da placa de circuito impresso foi o EAGLE. Este é um dos softwares de layout de PCBs mais conhecidos e utilizados no mundo. Extremamente popular entre hobbistas e estudantes de eletrônica, possui uma licença gratuita, na qual há limitações para o tamanho da placa e de número de páginas de esquemático. Para nossa aplicação, assim como o de muitos outros usuários, esta licença é mais do que suficiente.
...
A placa de interfaceamento é relativamente simples, basta conectar os seis pinos da placa de debug, conforme a figura(~\ref{fig:conector_ez430}) aos correspondentes pinos da placa EM430F6137RF900, como mostrado na imagem (~\ref{fig:jtag}). A sequência é a seguinte: GND-GND, 3.6V - Vcc, TEST-TST,RST-RST e os pinos TX e RX com uma barra de pinos que será ligada aos pinos P1.5 E P1.6 do CC430.
